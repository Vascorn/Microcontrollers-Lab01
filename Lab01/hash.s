...
.type hash, %function
	...
	*  \param R0 	pointer to the string to be hashed
	*  \param R1  	pointer to the array with the int hash value for every latin letter
	*/
	hash:
		.fnstart
		...
		loop:								
			LDRB R2, [R0], #1           // Load next character to R2 and R0 += 1 to show to next byte-character
			...
			LDRLS R4, [R1, R2, LSL #2]  // if R2 <= 25; load to R4 the hash value of current letter from R1
			ADDLS R3, R3, R4            // if R2 <= 25; R3 = R3 + R4 (add R4 to result)
			B loop							
		exit:								
